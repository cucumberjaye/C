...
static char	*one_line(char **str, int c)
{
	...
	if (c == 0)
		l = 1;
	while (i < (int)my_strlen(*str) + l)
	{
		if ((*str)[i] == '\n' || (*str)[i] == '\0')
		{
			res = my_strdup(*str, i + 1);
			new = *str;
			*str = my_strdup(*str + i + 1, my_strlen(*str + i + 1));
			free(new);
			*str = ft_realloc(*str, my_strlen(*str), c);
			i = 0;
			return (res);
		}
		++i;
